...
class EmbedMsg():
    ...
    async def send_embed_msg_inter(self, interaction, msg_title, msg_description, msg_color=discord.Color.blue(), follow_up=False, ephemeral=False):
        # If auto-delete is on
        if self.delete_msg_seconds and not ephemeral:
            msg_description += f"\n\nThis message will be deleted in {self.delete_msg_seconds} seconds."
        # Format the message
        msg_embed = discord.Embed(title=msg_title, description=msg_description, color=msg_color)
        ...
        if not follow_up:
            if ephemeral:
                await interaction.response.send_message(embed=msg_embed, ephemeral=ephemeral)
            else:
                await interaction.response.send_message(embed=msg_embed, delete_after=self.delete_msg_seconds)
        else:
            if ephemeral:
                message = await interaction.followup.send(embed=msg_embed, ephemeral=ephemeral)
            else:
